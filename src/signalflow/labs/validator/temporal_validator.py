...
from signalflow.labs._compat import validator
from signalflow.labs._compat import SignalValidator

from signalflow.labs._compat import Signals
...
from signalflow.labs.data.ts_preprocessor import TimeSeriesPreprocessor
...
class TemporalValidator(SignalValidator):
    ...
    def fit(
        ...
        if checkpoint_callback and checkpoint_callback.best_model_path:
            self.model = TemporalClassificator.load_from_checkpoint(checkpoint_callback.best_model_path)
    ...
    def _format_predictions(self, signals, prefix, all_probs):
        ...
        result_df = signals.value.with_columns([pl.Series(name, values) for name, values in prob_cols.items()])
